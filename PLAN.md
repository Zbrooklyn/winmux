...
- [ ] Tab overflow menu — chevron with a hidden-tab count, listing tabs that don't fit
      Uses the mockup's `.tab-of` / `.ofmenu` pattern; unread dot when a hidden tab needs you.
- [ ] Ctrl+Tab tab switching, MRU order, tab limit, and reopen-closed-tab
      Ctrl+Tab / Ctrl+Shift+Tab cycle most-recently-used; Ctrl+Shift+T reopens the last closed tab.
- [ ] Copy mode — keyboard scrollback selection with an on-screen hint bar
      The mockup's `.copymode` bar; arrows/PageUp move, Esc exits.
- [ ] Drag a tab to a pane edge to split, with a live split-preview overlay
      The mockup's `.dragging` / `.drop` / `.split-preview` classes.
- [ ] Per-tab busy underline — a progress line under a tab while its shell is producing output
      The mockup's `.tprog`.
- [ ] Attention ring on the pane that needs you, dimming the others
      The mockup's `.nring` / `.has-ring`, driven by the real terminal bell.
- [ ] Tab type icons — the shell's own icon per tab, not one generic prompt glyph
      PowerShell / Command Prompt / Git Bash / WSL each get their own mark.
- [ ] Pin a pane so layout changes leave it alone
      The mockup's `.ppin`; a pinned pane can't be closed by accident.
- [ ] Save/load layouts as an inline popover instead of a modal
      The mockup's `.sessmenu`: named entries, tab counts, age, delete ×. Same stored data.
- [ ] Half-width and phone layouts

...
const TOOLS = [
  ...
  { name: 'winmux_browser', desc: 'Drive the WinMux browser panel (Electron). sub: open|snapshot|click|back|forward|reload|url.',
    schema: { type: 'object', properties: { sub: { type: 'string' }, url: { type: 'string' }, ref: { type: 'string' } }, required: ['sub'] },
    cmd: 'browser' },

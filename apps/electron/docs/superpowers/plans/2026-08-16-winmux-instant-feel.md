...
- **IF-4 — Re-measure + prove.** Same probes, same machine; every fixed action shows its delta; nothing regressed (typing tick + startup re-run). Numbers to Edward. ✅ **Done 2026-08-16 as the v0.2.3 release gate:** the action table re-ran against the freshly installed 0.2.3 app — worst 32ms cold / 30ms warm across all 12 actions, launch→page 598ms warm; the webview browser leaf measured for the first time (chrome painted + webview attached in 16ms). Probes are committed assets (perf-actions/perf-installed/perf-echo); the release shipped with its numbers.
- **IF-5 — Perceived-instant polish.** Optimistic acknowledgment where real work takes time (pressed states, immediate overlay shells); only where IF-2 shows the wait is irreducible. **Disposition: nothing to build** — IF-2/IF-4 found no wait over 40ms anywhere in chrome, so there is nothing irreducible to acknowledge optimistically. The only >100ms-adjacent number (85ms typing echo) is the shared ConPTY path — the local-echo decision sits with Edward (IF-3 note).
- **IF-6 — Ship.** Harness green, release (0.2.3 or fold into next), Edward's hand-feel acceptance on his real machine is the exit gate — his "feels snappy" is the done-criterion, numbers are the evidence. **Shipped 2026-08-16: [v0.2.3](https://github.com/Zbrooklyn/winmux/releases/tag/v0.2.3) (438/438 green, four installers). Open: Edward's hand-feel sign-off + his named felt-slow moments, and the local-echo roadmap decision.**
...
**After IF-4/IF-6 ship (2026-08-16): 75/100** (Q2 7→8: the webview leaf measured — 16ms to painted chrome + attached webview, the last unmeasured surface closed; Q6 7→8: WebGL is default where it measurably helps and nothing measured would improve from more GPU work; Q9 5→8: v0.2.3 shipped WITH the re-run action table + echo numbers, probes committed). **Below 8, honestly:** Q5 stays 7 — the rendering-scales-with-pane structure is unchanged; every measurement says fixing it would be unfelt at real scale, so spending that rework is Edward's call, not a silent build. Q10 stays 3 until Edward's hand says so — that is this arc's designed exit gate and cannot be self-certified.
